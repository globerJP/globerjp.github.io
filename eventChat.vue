<template>
  <v-container v-if="!user">
    ...
    <v-container v-else-if="!account.firstName">
      <h1>
        You must be be properly registered to view this page.
      </h1>
      <v-btn
        color="#2196F3"
        class="pa-0"
...
export default {
    ...
    user() {
      return this.$store.state.authUser
		...
		screenHeight() {
			return this.isDevice ? window.innerHeight + 'px' : 'calc(100vh - 110px)'
		}
	},
	...
	async mounted() {
      ...
      await this.getUsers()
			...
			this.filesRef = storageRef.child('files')
    ...
    async getUsers() {
      ...
      this.allUsers.forEach(e => {
        const name = e.firstName +' '+ e.lastName +' '+e.id.substr(0,2)
       this.allUsersIdNameList.push({
          value: e.id,
          text: name
        })
